...
public class Q1UITest extends TestRun {
	...
	public void question1Test(){
		...
		float priceTotalExpected;
		float priceTotalActual;
		String sPricePerPerson;
		String sPricePerPersonExtra;
		//String sPriceTotalExpected;
		String sPriceTotalActual;
...
//			9.	Verify the payment amount as per the accommodation selected for 2 adults
		...
		accommodationPage.selectDoubleRoomType("1");
		accommodationPage.clickContinue();
		...
		//9.	Verify the payment amount as per the accommodation selected for 2 adults
		sPriceTotalActual = passengerDetailsPage.getTotalPrice();
		priceTotalActual = Float.parseFloat(sPriceTotalActual);

		priceTotalExpected = pricePerPerson *2;
		System.out.println("Price check=====================>");
		System.out.println("Price per person= '" + sPricePerPerson + "'");
		System.out.println("Expected Total Price should be= '" + String.valueOf(priceTotalExpected) + "'");
		System.out.println("Actual Total Price should be= '" + String.valueOf(priceTotalExpected) + "'");

		Assert.assertEquals(priceTotalExpected, priceTotalActual, "Expected price not equal to Actual" );
